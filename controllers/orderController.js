  ...
  try {
    ...
    const getOrderReview = async(req, res)=>{
      ...
      try {
        ...
        const validCoupons = coupons.filter(coupon => meetsCouponRequirements(coupon, cart, order));

        let totalDiscount = 0;
        for (const coupon of validCoupons) {
            totalDiscount += calculateCouponDiscount(coupon, cart, order);
        }
        console.log("coupons:::", coupons);
        if (cart && cart.products && cart.products.length > 0 && order && order.shippingAddress ) {
          ...
          res.render('orderreview', { user, cart: null, coupons, totalSubtotal: 0, defaultAddress: null, order: null  });
        }
      } catch (error) {
...
const showorder= async (req, res) => {
  try {
    const userId = req.session.user_id;
    const user = await User.findById({ _id: userId }); 
    const orders = await Order.find({ user: userId }).populate("user.shippingAddress");
